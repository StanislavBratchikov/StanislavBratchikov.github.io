...
***Methods:*** We have researched publicly available datasets and our own in-house data to compare CosMx and Xenium. For this purpose we used currently available analysis tools using python and R packages: ```python Squidpy```,```python Giotto```,```python stLearn```, ```python Spapros``` and ```python Seurat```. <br>
***Results:***  I have compared coverage of RNA transcripts to find platform that provides best transcript coverage. Additionaly I have estimated experiment and tissue specific batch effects and ways to integrate data preserving biological variability and reducing technical variation. Finally, I have studied multiple tools for targeted gene panel construction used in spatial transcriptomics. These tools allowed selecting 50 additional genes to add to existent commercial gene panels that would best preserve biological variation of human lung tissue.<br>
